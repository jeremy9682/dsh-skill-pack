---
name: ask-matt
description: Ask which skill or flow fits your situation. A router over the shared skills set plus the user's own skills.
---
...
Skills come from two places: the **shared skills set** this router ships with, and the **user's own skills**. The flows below name skills by their `/`-command; any one of them may live in the shared set or in the user's own set — if a name isn't present in the shared set, treat it as a skill to be found in the user's environment.
...
- **`/resolving-merge-conflicts`** — work an in-progress merge or rebase conflict hunk by hunk, resolving by **intent** traced to each side's primary source rather than by picking lines, then finish the operation. Locally overridden: if a hunk cannot be resolved with confidence it stops and asks; `--abort` stays available. (Upstream says it never runs `--abort`.) Standalone and off every flow: reach for it when you are already mid-conflict.
